...
enum ExprParsingState
{
    ExprParsingState_Term,
    ExprParsingState_BinOp,
};

internal expression_node* TryParseAddSubExpr(token* StartToken, token* LastToken)
{
    // NOTE: Result is the empty token which each iteration of the loop needs to fill out
    expression_node* Result = PushStruct(&SyntaxTreeArena, expression_node);
    *Result = {};

    u32 ExprParsingState = ExprParsingState_Term;
    ...
    while (CurrToken != LastToken)
    {
        ...
        {
            // TODO: This may only happen on the first iteration so move it outside the loop
            if (ExprParsingState != ExprParsingState_Term)
            {
                OutputError("Error: Expected to find a binary operator but found a term.\n");
            }

            expression_node* TermNode = PushStruct(&SyntaxTreeArena, expression_node);
            *TermNode = {};
            TermNode->Token = *CurrToken;

            Result->Left = TermNode;
            ExprParsingState = ExprParsingState_BinOp;
        }
        else if (CurrToken->Id == OpId_Add || CurrToken->Id == OpId_Sub)
        {
            ...
            if (!(CurrToken->Id == TokenId_Var ||  IsTokenConstant(*CurrToken)))
            {
                OutputError("Error: Expected term or constant.\n");
            }

            Result->OperatorId = OpId;
            Result->Right = PushStruct(&SyntaxTreeArena, expression_node);
            *Result->Right = {};
            Result->Right->Token = *CurrToken;
...
#if 0
internal expression_node* ParsePointerExprToTree(token* StartToken, token* LastToken)
{
    expression_node* Result = 0;
    token* CurrToken = StartToken;

    b32 IsLeftTerm = false;
    expression_node* Left = PushStruct(&SyntaxTreeArena, expression_node);
    *Left = {};
    while (CurrToken != LastToken)
    {
        if (IsTokenOperator(*CurrToken))
        {
            // NOTE: Look ahead for a token and to make sure we don't have a higher precedence op
            u32 OperatorId = CurrToken->Id;
            ++CurrToken;
            Assert(CurrToken->Id == TokenId_Var);
            if ((CurrToken + 1)->Id <= OperatorId)
            {
                expression_node* OpNode = PushStruct(&SyntaxTreeArena, expression_node);
                OpNode->Left = Left;
                OpNode->OperatorId = OperatorId;
                OpNode->Right = PushStruct(&SyntaxTreeArena, expression_node);
                *OpNode->Right = {};
                OpNode->Right->Token = *CurrToken;
            ...
            {
                if (Result && Result 
                Result = PushStruct(&SyntaxTreeArena, expression_node);
                Result->Left = Left;
        ...
        else if (CurrToken->Id == TokenId_Var || IsTokenConstant(*CurrToken))
        {
            if (IsLeftTerm)
            {
                OutputError("Error: Need to have operator between two terms.\n");
            }
            
            Left->Token = *CurrToken;
            IsLeftTerm = true;
        }
        else
        {
