...
inline virtual_reg* EvaluateBoolExpr(parser_state* State)
{
    ...
    virtual_reg* BoolExpr = SyntaxTreeToIR(State, ExprTree);
    ++State->StartToken;
...
inline b32 TryParseTypeStatement(u32 Flags, parser_state* State)
{
    if (State->StartToken->Id == TokenId_Type)
    {
        ...
        if (Flags & VarDefn_Allowed && State->StartToken->Id == ScannerId_EndLine)
        {
            ...
            {
                // TODO: Examine these and make sure its correct
                syntax_node* VarNode = CreateSyntaxNode(TokenId_Var, 0);
                VarNode->Token.Id = TokenId_Var;
                //VarNode->Token.Flags = TokenFlag_IsLValue;
        ...
        else if (Flags & FuncDecl_Allowed && State->StartToken->Id == OpId_OpenBracket)
        {
            ...
            State->Scope->FuncReturnType = Type;
            ...
            // NOTE: We need to let the type checking system know we are looking at a new function
            State->CurrNumLocals = 0;
        ...
        else if (Flags & VarDecl_Allowed && State->StartToken->Id == OpId_Equal)
        {
            ...
            // TODO: Examine these and make sure theyre correct
            syntax_node* VarNode = CreateSyntaxNode(TokenId_Var, 0);
            VarNode->Token.Id = TokenId_Var;
            //VarNode->Token.Flags = TokenFlag_IsLValue;
            VarNode->Token.Var = DefineVarInScope(State, NameStr, Type);
    ...
    if ((Flags & AllowReturn) != 0 && State->StartToken->Id == ScannerId_Return)
    {
        ...
        SyntaxTreeToIR(State, ReturnNode, State->Scope->FuncReturnType);            
        Assert(State->StartToken == State->LastToken);            
...
inline b32 TryParseWhile(parser_state* State)
{
    ...
    // TODO: We want our bool expr to either be 0 or not 0. Don't require a 1.
    // TODO: Push these strings on the scope arena?
    scope* WhileScope = AddScope(State, ScopeId_WhileLoop);
    WhileScope->StartHeaderName = GetWhileStartHeader(&TempArena, &NameState);
    WhileScope->ExitHeaderName = GetWhileExitHeader(&TempArena, &NameState);

    PushJumpHeaderInstr(WhileScope->StartHeaderName);
    // TODO: Make global strings of 1 and 0 since we use them a lot
    // TODO: FIX THIS 
    virtual_reg* BoolExpr = EvaluateBoolExpr(State);
    PushJumpCondInstr(BoolExpr, GetStringFromConstInt(&TempArena, 0, 1),
                      WhileScope->ExitHeaderName);
...
inline b32 TryParseCloseScope(parser_state* State)
{
    ...
    switch (State->Scope->Id)
    {
        ...
        {
            PushJumpInstr(State->Scope->StartHeaderName);
            PushJumpHeaderInstr(State->Scope->ExitHeaderName);
        } break;
    }
    
    // NOTE: We free the scope we allocated
    State->Scope->StructType = 0;
    State->Scope->FuncReturnType = 0;
