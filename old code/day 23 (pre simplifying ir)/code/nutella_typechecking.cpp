...
inline void GetRegisterFromToken(token Token, virtual_reg** OutReg)
{
    // NOTE: We set our accum register to the current token we are examining
    if (!IsTokenConstant(Token) && Token.Id == TokenId_Var)
    {
        (*OutReg) = GetOrCreateRegisterForVar(&RegHashTable, Token.Var);
    }
    else if (IsTokenConstant(Token))
    {
        (*OutReg) = GetTempRegister(&RegHashTable);
        (*OutReg)->Flags = RegFlag_StoresConstant | RegFlag_IsConstVal;
        (*OutReg)->Type = Token.Var->Type;
        (*OutReg)->Name = Token.Var->Str;
    }
    else
    {
        InvalidCodePath;
...
#if 1
inline void AccumRegisterFromToken(token Token, virtual_reg** OutReg)
{
    // NOTE: We set our accum register to the current token we are examining
    virtual_reg* Reg = 0;
    if (!IsTokenConstant(Token) && Token.Id == TokenId_Var)
    ...
    (*OutReg)->Flags = Reg->Flags;
    (*OutReg)->Type = Reg->Type;
    PushAssignInstr(*OutReg, Reg);
...
internal void SyntaxTreeToIR(parser_state* State, syntax_node* Root, virtual_reg** Accum = 0,
                             type* ReturnType = 0)
{
    // NOTE: To make calling simpler
    if (!Accum)
    {
        virtual_reg* _Accum;
        Accum = &_Accum;
        *Accum = GetTempRegister(&RegHashTable);
    }
    ...
    switch (Root->Id)
    {
        ...
        case OpId_Define:
        {
            ...
            (*Accum)->Flags |= RegFlag_IsLValue;
            (*Accum)->MemoryIndex = State->CurrNumLocals++;
            PushDefineInstr((*Accum));
            ...
            {
                PushAssignInstr((*Accum), RegFor0_Int);
            }
            else if (AreSameType((*Accum)->Type, GetBasicType(TypeId_Float)))
            {
                PushAssignInstr((*Accum), RegFor0_Flt);
            }
        } break;
        ...
        case OpId_Dereference:
        {
            ...
            if ((*Accum)->Flags & RegFlag_StoresConstant || !IsTypePointer((*Accum)->Type))
            {
                OutputError("Error: Cannot dereference a non pointer type.\n");
            }
            if (((*Accum)->Flags & RegFlag_IsLValue) == 0)
            {
            ...
            PushUniOpInstr(IR_LoadFromMem, (*Accum), (*Accum));
        } break;
        ...
        case OpId_AddressOff:
        {
            ...
            if ((*Accum)->Flags & RegFlag_StoresConstant)
            {
                OutputError("Error: Cannot take address of a constant.\n");
            }
                    
            (*Accum)->Type = ChangeRefCountToPtr((*Accum)->Type, 1);
            PushUniOpInstr(IR_AddressOff, (*Accum), (*Accum));
        } break;
        ...
        case OpId_Negate:
        {
            ...
            PushBinOpInstr(IR_Sub, RegFor0, (*Accum), (*Accum));
        } break;
        ...
        case OpId_GreaterEqual:            
        {
            ...
            else
            {
                ...
                Right = GetTempRegister(&RegHashTable);
            ...
            if ((*Accum)->Flags & RegFlag_StoresConstant && Right->Flags & RegFlag_StoresConstant)
            {
                (*Accum)->Flags |= RegFlag_StoresConstant;
            }

            // TODO: Do we have to keep the old type for accum here?
            PushBinOpInstr(IRCode, (*Accum), Right, (*Accum));
        } break;
        ...
        {            
            ...
            if (!((*Accum)->Flags & RegFlag_IsLValue))
            {
        ...
        case OpId_FuncCall:
        {
            ...
            for (u32 ArgIndex = 0; ArgIndex < Root->NumArgs; ++ArgIndex)
            {
                ...
                SyntaxTreeToIR(State, Root->Children[ArgIndex], ArgRegs + ArgIndex);
                
                if (!AreSameType(RequiredArgType, ArgRegs[ArgIndex]->Type))
                {
            ...
            PushFuncCallInstr((*Accum), Root->Func->Name, Root->NumArgs, ArgRegs);
