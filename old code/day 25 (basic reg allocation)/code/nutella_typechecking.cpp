...
internal void WhileStartToIR(virtual_reg* BoolReg, string StartHeader, string EndHeader)
{
    PushClearRegsInstr();
    PushJumpHeaderInstr(StartHeader);
    PushJumpCondInstr(BoolReg, RegFor0_Int->Reg, EndHeader);
}
...
internal void FuncEndToIR()
{
    PushFuncEndInstr();
}

internal typed_reg* SyntaxTreeToIR(syntax_node* Root, type* ReturnType = 0)
{
    typed_reg* Result = 0;
    ...
    switch (Root->Id)
    {
        ...
        case OpId_FuncCall:
        {
            ...
            for (u32 ArgIndex = 0; ArgIndex < Root->NumArgs; ++ArgIndex)
            {
                ...
                virtual_reg* ArgLHS = GetTempRegister(&RegHashTable);
                typed_reg* ArgReg = SyntaxTreeToIR(Root->Children[ArgIndex]);
                PushAssignInstr(ArgLHS, ArgReg->Reg);
                ArgRegs[ArgIndex] = ArgLHS;
            ...
            PushFuncCallInstr(Result->Reg, Root->Func->Name, Root->NumArgs, ArgRegs);
        ...
        case ScannerId_Return:
        {
            ...
            {
                token Token = SyntaxTreeToIR(Root->Children[0]);
