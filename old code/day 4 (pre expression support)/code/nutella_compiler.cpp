...
internal expression_node* ParseExpression(token* Tokens, u32 NumTokens)
{
    ...
    while (CurrToken <= (Tokens + NumTokens))
    {
        ...
        Result->Left = ParseExpression(CurrToken-1, 1);
        Result->Right = ParseExpression(CurrToken+1, NumTokens - 2);

        // TODO: This could just be infered from the token stored right now
        //Result->Id = ParseId_Add;
        break;
    }
    ...
    while (CurrTokenCount < NumTokens)
    {
        if (CurrToken->Id == TokenId_Name)
        {
            ...
            if (DeclareType)
            {
                ...
                {
                    var* SearchResult = FindVarNode(VarList, CurrToken->Text, CurrToken->NumChars);
                    if (!SearchResult)
                    {
