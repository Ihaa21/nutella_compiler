    ...
    switch (Root->Id)
    {
        ...
        {            
            ...
            if (LeftNode->Id == NodeId_Dereference)
            {
                ...
                Left = RecursiveSyntaxTreeToIR(Func, LeftNode->Children[0]);
                     ...
                     IsTypePointer(LeftNode->Children[0]->Var->Type))
            {
                ...
                Left = RecursiveSyntaxTreeToIR(Func, LeftNode->Children[0]);
                var* Temp = RecursiveSyntaxTreeToIR(Func, LeftNode->Children[1]);

                PushDefineInstr(Temp);
                PushMathInstr(IR_SubIntInt, Left,
                              CreateConstIntVar(GetStructMemOffset(Left->Type->PointerType, Temp)),
                              Left);
                PushAssignToMemInstr(Left, Right);
            }
            else
            {
                Left = RecursiveSyntaxTreeToIR(Func, LeftNode);
                if (IsTypeStruct(Left->Type))
                {
                    // NOTE: We have a struct copy so we have to copy all the elements
                    for (u32 MemberIndex = 0; MemberIndex < Left->Type->NumMembers; ++MemberIndex)
                    {
                        PushAssignInstr(Left->Members + MemberIndex, Right->Members + MemberIndex);
                    }
                }
                else
                {
