...
internal virtual_reg* RecursiveSyntaxTreeToIR(parser_state* State, syntax_node* Root, function* Func = 0)
{
    ...
    if (Root->Id == NodeId_Token)
    {
        ...
        {
            Result = GetOrCreateRegForToken(&RegHashTable, Root->Token);
        }
    ...
    switch (Root->Id)
    {
        ...
        case NodeId_Negate:
        {
            ...
            {
                RegFor0 = RegFor0_Int;
                PushMathInstr(IR_SubIntInt, RegFor0, Arg, Result);
            }
            else if (Arg->Type == GetBasicType(TypeId_Float))
            {
                RegFor0 = RegFor0_Flt;
                PushMathInstr(IR_SubFltFlt, RegFor0, Arg, Result);
            }            
...
internal void SyntaxTreeToIR(parser_state* State, syntax_node* Root)
{
    ...
    RecursiveSyntaxTreeToIR(State, Root);
}
