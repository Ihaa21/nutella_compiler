...
virtual_reg* RegFor0_Int;
virtual_reg* RegFor0_Flt;
...
int main(int argc, char** argv)
{
    ...
    RegHashTable.Array = PushArray(&RegArena, reg_hash_entry, RegHashTable.ArraySize);
    RegHashTable.RegArena = &RegArena;

    // NOTE: Setup 0 registers
    // TODO: Were to put in mem
    RegFor0_Int = PushStruct(&RegArena, virtual_reg);
    {
        char CharFor0 = '0';
        RegFor0_Int->Type = GetBasicType(TypeId_Int);
        RegFor0_Int->Name.NumChars = 1;
        RegFor0_Int->Name.Text = &CharFor0;
        RegFor0_Int->Flags = RegFlag_IsConstVal;
    }
    
    RegFor0_Flt = PushStruct(&RegArena, virtual_reg);
    {
        char CharFor0[] = "0.0";
        RegFor0_Flt->Type = GetBasicType(TypeId_Float);
        RegFor0_Flt->Name.NumChars = 3;
        RegFor0_Flt->Name.Text = CharFor0;
        RegFor0_Flt->Flags = RegFlag_IsConstVal;
    }
    
    FILE* OutFile = fopen(OutputFileName, "wb");
    FILE* NutellaFile = CompileFileToIR(FileName, GlobalScope, OutFile);
    //ConvertIrToAssembly(&IR_Arena, OutFile);

    syntax_node* Node = (syntax_node*)(SyntaxTreeArena.Mem);
