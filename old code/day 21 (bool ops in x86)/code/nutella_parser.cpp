...
inline b32 TryParseWhile(parser_state* State)
{
    ...
    WhileScope->StartHeaderName = GetWhileStartHeader(&TempArena);
    WhileScope->ExitHeaderName = GetWhileExitHeader(&TempArena);

    token BoolExpr = EvaluateBoolExpr(State);
    PushJumpHeaderInstr(WhileScope->StartHeaderName);
    PushJumpCondInstr(GetTokenString(BoolExpr), WhileScope->ExitHeaderName);
