...
// TODO: Tokens cannot store type ids anymore so move them over here instead
...
enum MemberFlag
{
    // NOTE: When we allocate members, we use this flag to know if we are a straight pointer
    // (in memory) to a type or if the type itself is of type pointer
    MemberFlag_IsPhysicalPtr,
    MemberFlag_IsTypePtr,
};
...
struct parser_state
{
    ...
    // NOTE: We use this value if we are in the scope of a struct
    type* StructType;
};
