...
internal expression_node* ParseExpressionToTree(token* Tokens, i32 NumTokens)
{
    ...
    while (CurrToken >= Tokens)
    {
        ...
        else if (CurrToken->Id == OpId_Mul || CurrToken->Id == OpId_Div)
        {
            if (CurrTokenCount == 0)
            {
...
int main(int argc, char** argv)
{
    ...
    {
        type* ArgTypeList[] = { GetBasicType(TypeId_Int),
                                GetBasicType(TypeId_Int) };
        type* ReturnType = GetBasicType(TypeId_Int);

        IntAddOp.OpId = OpId_Add;
        IntAddOp.NumArgs = 2;
