...
inline b32 TryParseElse(parser_state* State)
{
    ...
    if (State->StartToken->Id == ControlId_If)
    {
        ...
        syntax_node* ElseIfNode = CreateSyntaxNode(NodeId_ElseIf, 3);
        ...
        State->Scope->ExitCodeBlock = &ElseIfNode->Children[2];
        scope* Scope = AddScope(State, ScopeId_ElseIf);
        Scope->StartCodeBlock = &ElseIfNode->Children[1];
    }
    else
    {
        syntax_node* ElseNode = CreateSyntaxNode(NodeId_Else, 2);
        ...
        State->Scope->ExitCodeBlock = &ElseNode->Children[1];
