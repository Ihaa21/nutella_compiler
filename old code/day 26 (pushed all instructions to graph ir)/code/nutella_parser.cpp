...
inline void UpdateParserState(parser_state* State, syntax_node* Node)
{
    scope* Scope = State->Scope;

    if (State->Scope->ExitCodeBlock)
    {
        *State->Scope->ExitCodeBlock = Node;
...
inline b32 TryParseGeneralStatement(parser_state* State)
{
    ...
    UpdateParserState(State, ExprTree);
...
inline b32 TryParseTypeStatement(u32 Flags, parser_state* State)
{
    if (State->StartToken->Id == TokenId_Type)
    {
        ...
        if (Flags & VarDefn_Allowed && State->StartToken->Id == ScannerId_EndLine)
        {
            ...
            else
            {
                ...
                UpdateParserState(State, SetTo0Node);
        ...
        else if (Flags & FuncDecl_Allowed && State->StartToken->Id == OpId_OpenBracket)
        {
            ...
            // NOTE: Define our arugments in the scope
            for (u32 ArgIndex = 0; ArgIndex < Func->NumArgs; ++ArgIndex)
            {
                // TODO: Do we upadte the parser state here?
                syntax_node* VarNode = CreateSyntaxNode(NodeId_Var, 0);
                VarNode->Token.Id = TokenId_Var;
                ...
                //typed_reg* ArgReg = SyntaxTreeToIR(State, DefineNode);
        ...
        else if (Flags & VarDecl_Allowed && State->StartToken->Id == OpId_Equal)
        {
            ...
            UpdateParserState(State, EqualNode);
    ...
    if ((Flags & AllowReturn) != 0 && State->StartToken->Id == ScannerId_Return)
    {
        ...
        UpdateParserState(State, ReturnNode);
...
inline b32 TryParseIf(parser_state* State)
{
    ...
    syntax_node* IfNode = CreateSyntaxNode(NodeId_If, 3);
    ...
    State->Scope->ExitCodeBlock = &IfNode->Children[2];
