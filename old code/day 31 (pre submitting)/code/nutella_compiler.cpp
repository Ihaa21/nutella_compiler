...
/*
  TODO List:
      ...
      - We need to link up our different lines in the syntax tree so that each line points to the next.
        This way, we can have our if statements point to entire sub code blocks and when we run our
        #run statement, we have a clear idea of what the next statement in our code is. This also
        pushes all IR generation into our syntax tree to ir function. We need our function call nodes
        to also store the location of the syntax tree block for the actual function where calling. The
        reason is that when we run #run, we need to jump to different functions. Thus, we need our
        parser state to keep track of this info, like what we need to fill up next, if each code block
        needs to be stored somewhere (function call, if statement, etc.).
 */
    ...
    if (IsTypePointer(Type))
    {
        ...
        else
        {
            ...
            *Result = *Type;
            ...
            Result->SizeInBytes = PTR_SIZE_IN_BYTES;
        }
    }
    else
    {
...
inline u32 GetStructMemOffset(type* StructType, var* Member)
{    
    u32 MemoryIndex = 0;
    ...
    {
        CurrByte += sizeof(type*);
        string* Str = (string*)CurrByte;
        CurrByte += sizeof(string);

        if (StringCompare(Member->Str, *Str))
        {
            Found = true;
            break;
        }
...
inline void SetupStructVar(var* Var)
{
    Assert(IsTypeStruct(Var->Type));

    // TODO: Is this a good way to do struct types?
    // NOTE: We have a struct type so we need to create vars for its members that don't
    // conflict with regular variables names        
    Var->Members = PushArray(&VarArena, var, Var->Type->NumMembers);
    u8* CurrByte = Var->Type->Members;
    for (u32 MemberIndex = 0; MemberIndex < Var->Type->NumMembers; ++MemberIndex)
    {
        type* CurrArgType = *((type**)CurrByte);
        CurrByte += sizeof(type*);
        // TODO: We can remove the stirng now!!
        string* Str = (string*)CurrByte;
        CurrByte += sizeof(string);

        Var->Members[MemberIndex].Str = *Str;
        Var->Members[MemberIndex].Type = CurrArgType;
    }    
}
...
inline syntax_node* CreateSyntaxNode(u32 Id, u32 NumArgs, parser_state* State)
{
...
inline syntax_node* CreateSyntaxNodeFromToken(token* Token)
{
    syntax_node* Result = PushStruct(&SyntaxTreeArena, syntax_node);
    Result->Id = NodeId_Token;
    Result->Token = Token;
    Result->NumArgs = 0;
...
inline syntax_node* CreateUniSyntaxNode(u32 Id, syntax_node* Child, parser_state* State)
{
    syntax_node* Result = CreateSyntaxNode(Id, 1, State);
    Result->Children[0] = Child;
    
    return Result;
}

inline syntax_node* CreateBinSyntaxNode(u32 Id, syntax_node* LeftChild, syntax_node* RightChild,
                                        parser_state* State)
{
    syntax_node* Result = CreateSyntaxNode(Id, 2, State);
    Result->Children[0] = LeftChild;
    Result->Children[1] = RightChild;
...
inline var* GetTempVar()
{
    var* Result = PushStruct(&VarArena, var);
    Result->Flags = RegFlag_IsTemp;
    
    return Result;
}

inline var* CreateConstIntVar(i32 Val)
{
    var* Result = PushStruct(&VarArena, var);
    Result->Flags = RegFlag_IsConstVal | RegFlag_StoresConstant;
    Result->Str = ConvertIntToString(&VarArena, Val);
    
    return Result;
}

inline var* CreateConstFloatVar(f32 Val)
{
    var* Result = PushStruct(&VarArena, var);
    Result->Flags = RegFlag_IsConstVal | RegFlag_StoresConstant;
    Result->Str = ConvertFloatToString(&VarArena, Val);
...
inline string GetExitIfHeader(name_state* State)
{
    ...
    str_build_state StrState = StartBuildingString(&VarArena);
...
#include "nutella_interperter.cpp"
#include "nutella_ir.cpp"
#include "nutella_backend.cpp"
#include "nutella_typechecking.cpp"
#include "nutella_parser.cpp"

internal syntax_node* ScanFileToTokens(char* File, u32 NumChars, scope* GlobalScope, FILE* OutCFile)
{
    char* CurrChar = File;
    u32 CurrCharCount = 0;
    ...
    while (CurrChar <= (File + NumChars))
    {
        ...
        if (WasPrevMemberAccess)
        {
            ...
            CurrToken->Flags = TokenFlag_IsMember;
            
            DontChangeCurrChar = true;
            WasPrevMemberAccess = false;
        }
        else
        {
    ...
    // NOTE: Now that we parsed our code to a graph, we execute the #run code
    run_node_list* Curr = State.StartList;
    mem_arena Arena = InitArena(malloc(MegaBytes(2)), MegaBytes(2));    
    while (Curr)
    {
        ExecuteRunStatement(&Arena, Curr->Node);
        ClearArena(&Arena);
        Curr = Curr->Next;
    }
    free(Arena.Mem);
                               ...
                               syntax_node** Root)
{
    ...
    *Root = ScanFileToTokens(Data, (FileSizeInBytes - 1)/sizeof(char), GlobalScope, OutCFile);
...
int main(int argc, char** argv)
{
    ...
    OutputFileName[FileNameLen - 7] = 'a';
    OutputFileName[FileNameLen - 6] = 's';
    OutputFileName[FileNameLen - 5] = 'm';
    ...
    FILE* NutellaFile = CompileFileToIR(FileName, GlobalScope, OutFile, &Root);
    SyntaxTreeToIR(&NameState, Root);
    ConvertIrToAssembly(&IR_Arena, OutFile);
