...
internal virtual_reg* SyntaxTreeToIR(parser_state* State, syntax_node* Root, type* ReturnType = 0)
{
    ...
    if (Root->Id == TokenId_Var && Root->NumArgs == 0)
    {
        ...
        if (IsTokenConstant(Root->Token))
        {
            ...
            Result->Name = Root->Token.Var->Str;
        }
    ...
    switch (Root->Id)
    {
        case OpId_Define:
        {
            ...
            Result->Flags |= RegFlag_IsLValue;
            Result->MemoryIndex = State->CurrNumLocals++;
            ...
            {
                PushAssignInstr(Result, RegFor0_Int);
            }
            else if (AreSameType(Result->Type, GetBasicType(TypeId_Float)))
            {
                PushAssignInstr(Result, RegFor0_Flt);
            }
        } break;
        ...
        case OpId_Dereference:
        {
            ...
            Assert(Root->NumArgs == 1);
            virtual_reg* Ptr = SyntaxTreeToIR(State, Root->Children[0]);
        ...
        {
            Assert(Root->NumArgs == 1);
            virtual_reg* Val = SyntaxTreeToIR(State, Root->Children[0]);
        ...
        {
            Assert(Root->NumArgs == 1);
            virtual_reg* Arg = SyntaxTreeToIR(State, Root->Children[0]);
        ...
        case OpId_GreaterEqual:            
        {
            ...
            if (Left->Flags & RegFlag_StoresConstant && Right->Flags & RegFlag_StoresConstant)
            {
