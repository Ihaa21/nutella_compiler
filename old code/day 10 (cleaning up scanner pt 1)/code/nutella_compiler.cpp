...
global func_operator OperatorTable[5];
...
{
    Assert(Id > TokenId_StartTypes && Id < TokenId_EndTypes);
    type* Result = BasicTypes[Id - TokenId_StartTypes - 1];
    return Result;
}

inline b32 IsTokenConstant(token Token)
{
    b32 Result = (Token.Id == TokenId_ConstInt);
...
internal type* ResolveTypesForOp(u32 OpId, u32 NumArgs, type** ArgTypeList)
{
    type* RetType = 0;
    
    func_operator* CurrOp = OperatorTable;
    for (u32 OpIndex = 0; OpIndex < ArrayCount(OperatorTable); ++OpIndex, ++CurrOp)
    {
        if (CurrOp->OpId == OpId)
        {
            type** CurrType = CurrOp->ArgTypeList;
            if (CurrOp->NumArgs == NumArgs)
            {
                b32 MatchedTypes = true;
                for (u32 ArgId = 0; ArgId < CurrOp->NumArgs; ++ArgId)
                {
                    if (!(*CurrType == ArgTypeList[ArgId]))
                    {
                        MatchedTypes = false;
                        break;
                    }
...
internal token ExpressionTreeToIR(expression_node* Root, FILE* OutCFile)
{
    ...
    *ResultVarName.Var = {};
    ...
    else if (IsOpBinary(IR_OpId))
    {
        ...
        type* ReturnType = ResolveTypesForOp(Root->OperatorId, 2, ArgTypeList);
        if (!ReturnType)
        {
    ...
    while (CurrChar <= (File + NumChars))
    {
        ...
        else if (IsCharNumeric(*CurrChar))
        {
            ...
            if (CurrChar + NumNumbers == '.')
            {
                ...
                CurrToken->Id = TokenId_FloatConst;
            }
...
int main(int argc, char** argv)
{
    ...
    #if 0
    func_operator AddressOffOp = {};
    {
        type* ArgTypeList[] = { GetBasicType(TokenId_PointerType) };
        type* ReturnType = GetBasicType(TokenId_PointerType);

        PtrAddressOffOp.OpId = TokenId_AddressOff;
        PtrAddressOffOp.NumArgs = 2;
        PtrAddressOffOp.ArgTypeList = ArgTypeList;
        PtrAddressOffOp.ReturnType = ReturnType;
    }
#endif
    
    func_operator IntAddOp = {};
    {
        type* ArgTypeList[] = { GetBasicType(TokenId_ConstInt),
                                GetBasicType(TokenId_ConstInt) };
        type* ReturnType = GetBasicType(TokenId_ConstInt);

        IntAddOp.OpId = TokenId_Add;
        IntAddOp.NumArgs = 2;
        IntAddOp.ArgTypeList = ArgTypeList;
        IntAddOp.ReturnType = ReturnType;
    }

    func_operator IntSubOp = {};
    {
        type* ArgTypeList[] = { GetBasicType(TokenId_ConstInt),
                                GetBasicType(TokenId_ConstInt) };
        type* ReturnType = GetBasicType(TokenId_ConstInt);

        IntSubOp.OpId = TokenId_Sub;
        IntSubOp.NumArgs = 2;
        IntSubOp.ArgTypeList = ArgTypeList;
        IntSubOp.ReturnType = ReturnType;
    }

    func_operator IntMulOp = {};
    {
        type* ArgTypeList[] = { GetBasicType(TokenId_ConstInt),
                                GetBasicType(TokenId_ConstInt) };
        type* ReturnType = GetBasicType(TokenId_ConstInt);

        IntMulOp.OpId = TokenId_Mul;
        IntMulOp.NumArgs = 2;
        IntMulOp.ArgTypeList = ArgTypeList;
        IntMulOp.ReturnType = ReturnType;
    }

    func_operator IntDivOp = {};
    {
        type* ArgTypeList[] = { GetBasicType(TokenId_ConstInt),
                                GetBasicType(TokenId_ConstInt) };
        type* ReturnType = GetBasicType(TokenId_ConstInt);

        IntDivOp.OpId = TokenId_Div;
        IntDivOp.NumArgs = 2;
        IntDivOp.ArgTypeList = ArgTypeList;
        IntDivOp.ReturnType = ReturnType;
    }
    
    OperatorTable[0] = IntAddOp;
    OperatorTable[1] = IntSubOp;
    OperatorTable[2] = IntMulOp;
    OperatorTable[3] = IntDivOp;
