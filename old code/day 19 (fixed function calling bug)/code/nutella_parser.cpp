...
{    
    ...
    while (State->StartToken != LastToken)
    {
        if ((LoopState == -1 || LoopState == 0) && State->StartToken->Id == OpId_GetMember)
        {
            ...
            Result = CreateSyntaxNode(OpId_GetMember, 2);
            Result->Children[0] = Term;
            Result->Children[1] = CreateSyntaxNode(TokenId_Var, 0);
        ...
        else if ((LoopState == -1 || LoopState == 1) && State->StartToken->Id == OpId_OpenSqBracket)
        {
            ...
            Result = CreateSyntaxNode(OpId_GetArrayIndex, 2);
            Result->Children[0] = Term;
...
inline b32 TryParseTypeStatement(u32 Flags, parser_state* State)
{
    if (State->StartToken->Id == TokenId_Type)
    {
        ...
        else if (Flags & FuncDecl_Allowed && State->StartToken->Id == OpId_OpenBracket)
        {
            ...
            if (State->StartToken->Id != OpId_OpenBracket)
            {
                OutputError("Error: No brackets following function definition.\n");
            }
            ++State->StartToken;
            ...
            // NOTE: We need to put our pointer types onto the scope arena before we create our
            // function scope so that those types don't get cleared when we finish our func scope
            u32 NumArgs = 0;
            temp_mem TempMem = BeginTempMem(&TempArena);
            var* TempArgArray = (var*)(TempArena.Mem + TempArena.Used);
            ...
            while (State->StartToken != EndBracketToken)
            {
                ...
                var* TempArg = PushStruct(&TempArena, var);
                TempArg->Type = ArgType;
                TempArg->Str = State->StartToken->Str;
                ++State->StartToken;
                ++NumArgs;
            ...
            // NOTE: Now that our arg types are properly pushed and our scope is defined,
            // we define our arguments in our scope
            for (u32 ArgIndex = 0; ArgIndex < NumArgs; ++ArgIndex)
            {
                var* TempVar = TempArgArray + ArgIndex;
                var* Arg = DefineVarInScope(State, TempVar->Str, TempVar->Type);
                PushDefineInstr(TempVar->Type, TempVar->Str);                
            }
        ...
        else if (Flags & VarDecl_Allowed && State->StartToken->Id == OpId_Equal)
        {
            ...
            syntax_node* LHS_Node = CreateSyntaxNode(TokenId_Var, 0);
            LHS_Node->Token.Id = TokenId_Var;
            LHS_Node->Token.Flags = TokenFlag_IsLValue;
            LHS_Node->Token.Var = DefineVarInScope(State, NameStr, Type);
            PushDefineInstr(Type, NameStr);

            syntax_node* EqualNode = CreateSyntaxNode(OpId_Equal, 2);
            EqualNode->Children[0] = LHS_Node;
...
inline b32 TryParseStruct(parser_state* State)
{
    ...
    PushStructInstr(StructType->Str);
