...
{    
    ...
    while (State->StartToken != LastToken)
    {
        if (State->StartToken->Id == OpId_GetMember)
        {
            ...
            syntax_node* NewResult = CreateSyntaxNode(OpId_GetMember, 2);
            NewResult->Children[0] = Result;
            NewResult->Children[1] = CreateSyntaxNode(TokenId_Var, 0);
        ...
        else if ((!WasPrevIndex) && State->StartToken->Id == OpId_OpenSqBracket)
        {
            ...
            syntax_node* NewResult = CreateSyntaxNode(OpId_GetArrayIndex, 2);
            NewResult->Children[0] = Result;
...
inline syntax_node* TryParseTerm(parser_state* State, token* LastToken)
{
    ...
    while ((!FoundTerm) && State->StartToken != LastToken)
    {
        ...
        else if (State->StartToken->Id == OpId_Run)
        {
            ...
            *CurrNode = CreateSyntaxNode(OpId_Run, 1);
        ...
        else if (State->StartToken->Id == TokenId_Func)
        {
            ...
            *CurrNode = CreateSyntaxNode(OpId_FuncCall, Func->NumArgs);
            (*CurrNode)->Func = Func;
        ...
        {
            *CurrNode = CreateSyntaxNode(TokenId_Var, 0);
...
inline virtual_reg* EvaluateBoolExpr(parser_state* State)
{
    if (State->StartToken->Id != OpId_OpenBracket)
    {
    ...
    typed_reg* BoolExpr = SyntaxTreeToIR(ExprTree);
    ++State->StartToken;
...
inline b32 TryParseTypeStatement(u32 Flags, parser_state* State)
{
    if (State->StartToken->Id == TokenId_Type)
    {
        ...
        if (Flags & VarDefn_Allowed && State->StartToken->Id == ScannerId_EndLine)
        {
            ...
            {
                syntax_node* VarNode = CreateSyntaxNode(TokenId_Var, 0);
                VarNode->Token.Id = TokenId_Var;
                VarNode->Token.Var = DefineVarInScope(State, NameStr, Type);
                
                syntax_node* DefineNode = CreateSyntaxNode(OpId_Define, 1);
                DefineNode->Children[0] = VarNode;

                SyntaxTreeToIR(DefineNode);
                
                syntax_node* SetTo0Node = CreateSyntaxNode(OpId_Zero, 1);
                SetTo0Node->Children[0] = VarNode;

                SyntaxTreeToIR(SetTo0Node);
            }
        ...
        else if (Flags & FuncDecl_Allowed && State->StartToken->Id == OpId_OpenBracket)
        {
            ...
            for (u32 ArgIndex = 0; ArgIndex < Func->NumArgs; ++ArgIndex)
            {
                syntax_node* VarNode = CreateSyntaxNode(TokenId_Var, 0);
                VarNode->Token.Id = TokenId_Var;
                VarNode->Token.Var = ArgArray + ArgIndex;
                
                syntax_node* DefineNode = CreateSyntaxNode(OpId_Define, 1);
                DefineNode->Children[0] = VarNode;

                typed_reg* ArgReg = SyntaxTreeToIR(DefineNode);
                ArgReg->Reg->MemoryIndex = ArgIndex*4 + 8;
            }
        ...
        else if (Flags & VarDecl_Allowed && State->StartToken->Id == OpId_Equal)
        {
            ...
            syntax_node* EqualNode = CreateSyntaxNode(OpId_Equal, 2);
            EqualNode->Children[0] = DefineNode;
            EqualNode->Children[1] = ExprTree;
            
            SyntaxTreeToIR(EqualNode);
            Assert(State->StartToken == State->LastToken);            
    ...
    if ((Flags & AllowReturn) != 0 && State->StartToken->Id == ScannerId_Return)
    {
        ...
        if (State->StartToken == State->LastToken)
        {        
            ReturnNode = CreateSyntaxNode(ScannerId_Return, 0);            
        }
        else
        {        
            ReturnNode = CreateSyntaxNode(ScannerId_Return, 1);
...
inline b32 TryParseIf(parser_state* State)
{
    ...
    // NOTE: Let the upper scope know we contain a if statement
    State->Scope->ExitIfStatement = GetExitIfHeader(&TempArena, &NameState);
        
    virtual_reg* BoolReg = EvaluateBoolExpr(State);
    ...
    State->Scope->ExitLocalIfHeader = GetExitIfHeader(&TempArena, &NameState);
    IfStartToIR(BoolReg, State->Scope->ExitLocalIfHeader);
    ++State->StartToken;
    Assert(State->StartToken == State->LastToken);
    return true;
}
...
inline b32 TryParseWhile(parser_state* State)
{
    ...
    scope* WhileScope = AddScope(State, ScopeId_WhileLoop);
    WhileScope->StartHeader = GetWhileStartHeader(&RegArena, &NameState);
    WhileScope->ExitHeader = GetWhileExitHeader(&RegArena, &NameState);

    PushClearRegsInstr();
    PushJumpHeaderInstr(WhileScope->StartHeader);
    virtual_reg* BoolExpr = EvaluateBoolExpr(State);
    PushJumpCondInstr(BoolExpr, RegFor0_Int->Reg, WhileScope->ExitHeader);
    if (!(State->StartToken->Id == ScannerId_OpenScope))
    {
...
inline b32 TryParseCloseScope(parser_state* State)
{
    ...
    switch (State->Scope->Id)
    {
        case ScopeId_If:
        case ScopeId_ElseIf:
        {
            IfEndToIR(State->Scope->ExitLocalIfHeader, State->Scope->PrevScope->ExitIfStatement);
        } break;
        
        case ScopeId_WhileLoop:
        {
            WhileEndToIR(State->Scope->StartHeader, State->Scope->ExitHeader);
        } break;

        case ScopeId_Function:
        {
            FuncEndToIR(State->Scope->Func);
        } break;
    }
    
    // NOTE: We free the scope we allocated
    State->Scope->StartHeader = {};
    State->Scope->ExitHeader = {};
...
internal parser_state* ParseTokens(parser_state* State)
{
    ...
    else if (IsScopeCodeBlock(State->Scope->Id))
    {
        ...
        if (State->Scope->ExitIfStatement.Text)
        {
            // NOTE: We have to close our if statement
            ElseEndToIR(State->Scope->ExitIfStatement);
        }
