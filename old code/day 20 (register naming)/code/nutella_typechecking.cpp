...
inline string GetRegisterNameForVar(reg_hash_table* Table, string Name)
{
    ...
    if (Entry->EntryName.NumChars == 0)
    {
        ...
        // TODO: Generate a register name
        Entry->RegName = GetRegisterName(&RegNameArena, Table->CurrRegCount++);
    ...
    else
    {
        ...
        Entry->Next = PushStruct(&RegNameArena, reg_hash_entry);
        Entry = Entry->Next;
        Entry->EntryName = Name;
        Entry->RegName = GetRegisterName(&RegNameArena, Table->CurrRegCount++);
...
{
    string Result = GetRegisterName(&ScopeArena, Table->CurrRegCount++);
    return Result;
}
...
internal token SyntaxTreeToIR(syntax_node* Root, FILE* OutCFile, type* ReturnType = 0)
{
    ...
    token ResultVarName = {};
    if (Root->Id != OpId_Define)
    {
        ResultVarName.Id = TokenId_Var;
        ResultVarName.Var = PushStruct(&ScopeArena, var);
        ResultVarName.Var->Str = InitStr((char*)VarNames[UsedCount], (u32)strlen(VarNames[UsedCount]));
        //ResultVarName.Var->Str = GetRegisterNameForVar();
        ++UsedCount;
    }
