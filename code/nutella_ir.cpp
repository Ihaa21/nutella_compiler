...
internal void PushFuncCallInstr(var* Result, function* Func, u32 NumArgs, var** Args)
{
    ir_instr Instr = {};
...
internal void RecursiveDefineVar(var* Var, b32 IsArg)
{    
    if (!IsTypeStruct(Var->Type))
    {
        if (IsArg)
        {
            Var->Flags |= RegFlag_IsArg;
        }
        PushDefineInstr(Var);
    }
    else
    {
        ...
        {
            RecursiveDefineVar(Var->Members + MemberIndex, IsArg);
        }
    }
}
...
internal void RecursivePushArg(var* Var)
{
    if (!IsTypeStruct(Var->Type))
    {
        var** ArrayArg = PushStruct(&TempArena, var*);
        ArrayArg[0] = Var;
    }
    else
    {
        for (u32 MemberIndex = 0; MemberIndex < Var->Type->NumMembers; ++MemberIndex)
        {
            RecursivePushArg(Var->Members + MemberIndex);
        }
    }    
}
...
internal var* RecursiveSyntaxTreeToIR(function* Func, syntax_node* Root)
{
    ...
    switch (Root->Id)
    {
        ...
        case NodeId_Define:
        {
            ...
            RecursiveDefineVar(Result, Result->Flags & RegFlag_IsArg);
        } break;
        ...
        {            
                     ...
                     IsTypePointer(LeftNode->Children[0]->Var->Type))
            {
                ...
                RecursiveDefineVar(MemberAddr, 0);
        ...
        case NodeId_FuncCall:
        {
            ...
            var** ArgRegs = (var**)(TempArena.Mem + TempArena.Used);
            for (u32 ArgIndex = 0; ArgIndex < Root->NumArgs; ++ArgIndex)
            {
                var* ArgReg = RecursiveSyntaxTreeToIR(Func, Root->Children[ArgIndex]);
                RecursivePushArg(ArgReg);
            }

            // TODO: If no return value, make result empty
            PushFuncCallInstr(Result, Root->Func, (u32)(((mm)(TempArena.Mem + TempArena.Used) - (mm)ArgRegs)/sizeof(var*)),
                              ArgRegs);
