...
{    
    ...
    while (State->StartToken != LastToken)
    {
        if (State->StartToken->Id == OpId_GetMember)
        {
            ...
            syntax_node* VarNode = CreateSyntaxNodeFromToken(State->StartToken);
            Result = CreateBinSyntaxNode(NodeId_GetMember, Result, VarNode, 0);
        ...
        else if ((!WasPrevIndex) && State->StartToken->Id == OpId_OpenSqBracket)
        {
            ...
            Result = CreateBinSyntaxNode(NodeId_GetArrayIndex, Result, IndexNode, 0);
            WasPrevIndex = true;
        }
        else
        {
...
inline syntax_node* TryParseTerm(parser_state* State, token* LastToken)
{
    ...
    while ((!FoundTerm) && State->StartToken != LastToken)
    {
            ...
            State->StartToken->Id == OpId_Sub)
        {
            ...
            *CurrNode = CreateSyntaxNode(OpId, 1, 0);
        ...
        else if (State->StartToken->Id == OpId_Run)
        {
            ...
            *CurrNode = CreateUniSyntaxNode(NodeId_Run, ParseExpr(State, LastToken), 0);
        ...
        else if (State->StartToken->Id == TokenId_Func)
        {
            ...
            *CurrNode = CreateSyntaxNode(NodeId_FuncCall, Func->NumArgs, 0);
            (*CurrNode)->Func = Func;
        ...
        {
            *CurrNode = CreateSyntaxNodeFromToken(State->StartToken);
            ++State->StartToken;
                                         ...
                                         syntax_node* FirstTerm)
{
    ...
    while (State->StartToken != LastToken)
    {
        if (State->StartToken->Id == OpId_Mul || State->StartToken->Id == OpId_Div)
        {
            ...
            Result = CreateBinSyntaxNode(OpId, Result, TermNode, 0);            
        }
                                             ...
                                             syntax_node* FirstTerm)
{
    ...
    while (State->StartToken != LastToken)
    {
        if (State->StartToken->Id == OpId_Add || State->StartToken->Id == OpId_Sub)
        {
            ...
            syntax_node* Child2 = TryParseTerm(State, LastToken);
            if (State->StartToken != State->LastToken &&
                IsTokenBinOp(*State->StartToken) && State->StartToken->Id > OpId_Sub)
            {
                ...
                Child2 = TryParseMulDivExpr(State, LastToken, Child2);
            }
            
            Result = CreateBinSyntaxNode(OpId, Result, Child2, 0);            
        }
        else if (IsTokenBinOp(*State->StartToken))
        {
    ...
    while (State->StartToken != LastToken)
    {
            ...
            State->StartToken->Id == OpId_Greater || State->StartToken->Id == OpId_GreaterEqual)
        {
            ...
            if (IsTokenBinOp(*State->StartToken) || State->StartToken->Id > OpId_GreaterEqual)
            {
                ...
                Child2 = TryParseAddSubExpr(State, LastToken, Child2);
            }
...
internal syntax_node* ParseEqualExpr(parser_state* State)
{
    ...
    while (State->StartToken != State->LastToken)
    {
        if (!EncounteredEquals && State->StartToken->Id == OpId_Equal)
        {
            ...
            syntax_node* Child2 = TryParseTerm(State, State->LastToken);
            if (State->StartToken != State->LastToken &&
                IsTokenBinOp(*State->StartToken) && State->StartToken->Id > OpId_Equal)
            {
                Child2 = ParseExpr(State, State->LastToken, Child2);
            }
...
inline b32 TryParseGeneralStatement(parser_state* State)
{
    ...
    TypeCheckSyntaxTree(State, ExprTree);
...
inline b32 TryParseTypeStatement(u32 Flags, parser_state* State)
{
    if (State->StartToken->Id == TokenId_Type)
    {
        ...
        if (Flags & VarDefn_Allowed && State->StartToken->Id == ScannerId_EndLine)
        {
            ...
            {
                syntax_node* VarNode = CreateSyntaxNodeFromToken(NameToken);
                VarNode->Token->Id = TokenId_Var;
                VarNode->Token->Var = DefineVarInScope(State, NameStr, Type);
                syntax_node* DefineNode = CreateUniSyntaxNode(NodeId_Define, VarNode, State);
                TypeCheckSyntaxTree(State, DefineNode);
                
                syntax_node* SetTo0Node = CreateUniSyntaxNode(NodeId_Zero, VarNode, State);
                TypeCheckSyntaxTree(State, SetTo0Node);
            }
        ...
        else if (Flags & FuncDecl_Allowed && State->StartToken->Id == OpId_OpenBracket)
        {
            ...
            Func->StartNode = CreateSyntaxNode(NodeId_FuncDef, 1, State);
            Func->StartNode->Func = Func;
            TypeCheckSyntaxTree(State, Func->StartNode);
            ...
            while (State->StartToken != EndBracketToken)
            {
                ...
                token* ArgToken = State->StartToken;
                var* Arg = DefineVarInScope(State, GetTokenString(*ArgToken), ArgType);
                ++State->StartToken;

                syntax_node* VarNode = CreateSyntaxNodeFromToken(ArgToken);
                VarNode->Token->Var = Arg;
                VarNode->Token->Id = TokenId_Var;
                VarNode->Token->Flags |= TokenFlag_IsArg;
                syntax_node* DefineNode = CreateUniSyntaxNode(NodeId_Define, VarNode, State);
                TypeCheckSyntaxTree(State, DefineNode);
        ...
        else if (Flags & VarDecl_Allowed && State->StartToken->Id == OpId_Equal)
        {
            ...
            VarNode->Token->Id = TokenId_Var;
            VarNode->Token->Var = DefineVarInScope(State, NameStr, Type);
            syntax_node* DefineNode = CreateUniSyntaxNode(NodeId_Define, VarNode, 0);
            syntax_node* EqualNode = CreateBinSyntaxNode(NodeId_Equal, DefineNode, ExprTree, State);
            TypeCheckSyntaxTree(State, EqualNode);
    ...
    if ((Flags & AllowReturn) != 0 && State->StartToken->Id == ScannerId_Return)
    {
        ...
        if (State->StartToken == State->LastToken)
        {        
            ReturnNode = CreateSyntaxNode(NodeId_Return, 0, 0);            
        }
        else
        {        
            ReturnNode = CreateSyntaxNode(NodeId_Return, 1, 0);
            ReturnNode->Children[0] = ParseExpr(State);
        }

        State->IsPrevIf = false;
        TypeCheckSyntaxTree(State, ReturnNode);            
        Assert(State->StartToken == State->LastToken);            
...
inline b32 TryParseIf(parser_state* State)
{
    ...
    syntax_node* IfNode = CreateSyntaxNode(NodeId_If, 2, 0);
    IfNode->Children[0] = EvaluateBoolExpr(State);
    TypeCheckSyntaxTree(State, IfNode);
...
inline b32 TryParseElse(parser_state* State)
{
    ...
    if (State->StartToken->Id == ControlId_If)
    {
        ...
        syntax_node* ElseIfNode = CreateSyntaxNode(NodeId_ElseIf, 2, 0);
        ElseIfNode->Children[0] = EvaluateBoolExpr(State);
        TypeCheckSyntaxTree(State, ElseIfNode);
    ...
    {
        syntax_node* ElseNode = CreateSyntaxNode(NodeId_Else, 1, 0);
        TypeCheckSyntaxTree(State, ElseNode);
...
inline b32 TryParseWhile(parser_state* State)
{
    ...
    syntax_node* WhileNode = CreateSyntaxNode(NodeId_While, 2, 0);
    WhileNode->Children[0] = EvaluateBoolExpr(State);
    TypeCheckSyntaxTree(State, WhileNode);
    ...
    State->Scope->ExitCodeBlock = &WhileNode->Next;
...
internal parser_state* ParseTokens(parser_state* State)
{
    ...
    // NOTE: This never happens, VS just complains about not all paths returning
    return 0;
}
