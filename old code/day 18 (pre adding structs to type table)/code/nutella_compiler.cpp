...
inline type* GetBasicType(u32 Id)
{
    ...
    type* Result = ArenaMem + (Id - TypeId_StartTypes - 1);
    return Result;
}
...
internal type* IsTypeDefined(string MatchStr)
{
    ...
    while (CurrByte < (TypeArena.Mem + TypeArena.Used))
    {
        ...
        CurrByte += sizeof(type);// + sizeof(type*)*CurrFunc->NumArgs;
    }
...
inline b32 AreSameType(type* Type1, type* Type2)
{
    ...
    Assert(!IsTypePointer(Type1) || (IsTypePointer(Type1) && Type1->NumRef > 0 && Type1->PointerType->NumRef == 0));
    Assert(!IsTypePointer(Type2) || (IsTypePointer(Type2) && Type2->NumRef > 0 && Type2->PointerType->NumRef == 0));
