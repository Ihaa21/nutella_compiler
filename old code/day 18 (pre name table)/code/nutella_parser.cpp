...
inline syntax_node* TryParseTerm(parser_state* State, token* LastToken)
{
    ...
    // NOTE: We check here for accessing members
    while (State->StartToken != LastToken)
    {
        if (State->StartToken->Id == OpId_GetMember)
        {
            // TODO: Should we have a flag that says this type is a struct type?
            // We need to allow our name to be anything, even prev defined types but
            // not basic type names.
            ++State->StartToken;
            ...
            NewResult->OperatorId = OpId_GetMember;
            NewResult->Children[0] = Result;
            NewResult->Children[1] = CreateSyntaxNode(0);
            NewResult->Children[1]->OperatorId = TokenId_Var;
    ...
    // NOTE: We check here for array accessing
    if (State->StartToken->Id == OpId_OpenSqBracket)
    {
        ...
        syntax_node* ArrayAccess = CreateSyntaxNode(2);
        ArrayAccess->OperatorId = OpId_GetArrayIndex;
        ArrayAccess->Children[0] = Result;
        ArrayAccess->Children[1] = IndexNode;
