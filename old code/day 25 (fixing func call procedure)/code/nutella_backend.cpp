    ...
    while (State.CurrByte < ((u8*)IR_Arena->Mem + IR_Arena->Used))
    {
        ...
        switch(Instr->Id)
        {
            ...
            case IR_AddressOff:
            {
                ...
                Result->Flags |= RegFlag_LoadToMem;
            ...
            case IR_FuncCall:
            {
                ...
                if (HasReturnVal)
                {
                    ...
                    virtual_reg* ReturnVal = State.LoadedRegisters[EAX_REG_INDEX];
                    AllocateReg(ReturnVal, &State);
                    AllocateReg(Result, &State);
                    x86_WriteMov(Result->Name, ReturnVal->Name, &State);
