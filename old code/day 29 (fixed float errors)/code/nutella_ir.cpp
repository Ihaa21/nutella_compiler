...
internal virtual_reg* RecursiveSyntaxTreeToIR(function* Func, syntax_node* Root)
{
    ...
    switch (Root->Id)
    {
        case NodeId_Reg:
        {
            ...
            if (Root->Reg->Flags & RegFlag_IsConstVal &&
                AreSameType(Result->Type, GetBasicType(TypeId_Float)))
            {
                f32 Val = ConvertStringToFloat(Result->Name);
        ...
        case NodeId_Dereference:
        {
            ...
            Assert(Root->NumArgs == 1);
            virtual_reg* Ptr = RecursiveSyntaxTreeToIR(Func, Root->Children[0]);
        ...
        {
            Assert(Root->NumArgs == 1);
            virtual_reg* Val = RecursiveSyntaxTreeToIR(Func, Root->Children[0]);
        ...
        {
            Assert(Root->NumArgs == 1);
            virtual_reg* Arg = RecursiveSyntaxTreeToIR(Func, Root->Children[0]);
        ...
        case NodeId_GreaterEqual:            
        {
            ...
            virtual_reg* Left = RecursiveSyntaxTreeToIR(Func, Root->Children[0]);
            virtual_reg* Right = RecursiveSyntaxTreeToIR(Func, Root->Children[1]);
        ...
        case NodeId_FuncCall:
        {
            ...
            {
                virtual_reg* ArgReg = RecursiveSyntaxTreeToIR(Func, Root->Children[ArgIndex]);
    ...
    while (CurrNode)
    {
        switch (CurrNode->Id)
        {
            case NodeId_If:
            {
                ...
                do
                {
                    virtual_reg* BoolReg = RecursiveSyntaxTreeToIR(Func, CurrNode->Children[0]);
                    PushClearRegsInstr();
            ...
            case NodeId_While:
            {
                ...
                PushClearRegsInstr();
                virtual_reg* BoolReg = RecursiveSyntaxTreeToIR(Func, Root->Children[0]);
