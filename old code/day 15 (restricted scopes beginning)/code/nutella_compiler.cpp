...
inline b32 ScopeAllowsVarManip(u32 Id)
{
...
inline token EvaluateBoolExpr(token* CurrToken, u32 NumScannedTokens, u32 NumTokens,
                              FILE* OutFile, token** FinishedToken)
{    
    if (CurrToken->Id != OpId_OpenBracket)
    {
    ...
    u32 BoolExprCount = 0;
    for (; BoolExprCount < NumTokens - NumScannedTokens; ++BoolExprCount, ++CurrToken)
    {
        if (CurrToken->Id == OpId_CloseBracket)
        {
            break;
...
internal void ParseTokens(token* TokenArray, u32 NumTokens, FILE* OutFile)
{
    ...
    if (CurrToken->Id == ScannerId_Identifier)
    {
        ...
        if (Type)
        {
            ...
            {
                // NOTE: Here we know we are looking at a variable
            ...
            {
                // NOTE: Here we know we are looking at a function
                ...
                u32 CurrentState = 0;
                type* PrevType = 0;
                while (CurrToken != (TokenArray + NumTokens))
                {
                    if (CurrentState == ArgParseState_Type)
                    {
                        ...
                        PrevType = IsTypeDefined(GlobalCurrScope, CurrToken->Str);
                        if (!PrevType)
                        {
                    ...
                    else if (CurrentState == ArgParseState_VarName)
                    {
                        ...
                        var* Arg = IsVarDefined(GlobalCurrScope, CurrToken->Str);
                        if (Arg)
                        {
                        ...
                        Arg = DefineVarInScope(GlobalCurrScope, CurrToken->Str, PrevType);
                ...
                ++CurrToken;
                ParseTokens(CurrToken,
                            (u32)((u64)(TokenArray + NumTokens) - (u64)CurrToken) / sizeof(token),
                            OutFile);
            }
            else if (CurrToken->Id == OpId_Equals)
            {
                ...
                u32 NewNumTokens = NumElementsBetweenIndexes(TokenArray + NumTokens, CurrToken);
        ...
        if (ScopeAllowsVarManip(GlobalCurrScope->Id) && LHS_Var)
        {            
            ++CurrToken;
    ...
    else if (ScopeAllowsVarManip(GlobalCurrScope->Id) && CurrToken->Id == ControlId_If)
    {
        ++CurrToken;
        
        token BoolExpr = EvaluateBoolExpr(CurrToken, 1, NumTokens, OutFile, &CurrToken);
    ...
    else if (ScopeAllowsVarManip(GlobalCurrScope->Id) && CurrToken->Id == ControlId_Else)
    {
        ++CurrToken;
    ...
    else if (ScopeAllowsVarManip(GlobalCurrScope->Id) && CurrToken->Id == ControlId_While)
    {
        ++CurrToken;
    ...
    else if (GlobalCurrScope->Id == ScopeId_Global && CurrToken->Id == ScannerId_Struct)
    {
        ++CurrToken;
