...
internal var* RecursiveTypeCheckSyntaxTree(parser_state* State, syntax_node* Root)
{
    ...
    if (Root->Id == NodeId_Token)
    {
        ...
        if (Root->Token->Flags & TokenFlag_IsArg)
        {
    ...
    switch (Root->Id)
    {
        ...
        case NodeId_Dereference:
        {
            ...
            else if (IsTypeStruct(Result->Type))
            {
                SetupStructVar(Result);
            }
        } break;
        ...
        case NodeId_GetMember:
        {
            ...
            {
                if (!IsTypeStruct(Left->Type->PointerType))
                {
                    OutputError("Error: Attempting to access member in non struct type.\n");
                }
                if (Left->Type->NumRef > 1)
                {
                    OutputError("Error: Attempting to access member in non struct type.\n");
                }
            ...
            else
            {
                ...
                {
                    var* Member = Left->Members + MemberIndex;
