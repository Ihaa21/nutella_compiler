...
inline virtual_reg* GetOrCreateRegForToken(reg_hash_table* Table, token* Token)
{
    if (Token->Var->Reg)
    {
...
inline virtual_reg* GetOrCreateConstIntRegister_(reg_hash_table* Table, u32 Val)
{
    temp_mem TempMem = BeginTempMem(&RegArena);
    string Str = ConvertIntToString(&RegArena, Val);
    virtual_reg* Result = 0;//GetOrCreateRegister(Table, Str);
    if (Result->Flags == 0)
    {
